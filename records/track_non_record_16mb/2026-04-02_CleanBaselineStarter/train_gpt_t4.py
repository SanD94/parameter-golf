"""
T4-adapted version of train_gpt.py for Google Cloud free-tier T4 GPUs.

This file is auto-generated from train_gpt.py with the following adaptations:
  - bfloat16 → float16 (T4 has no native bf16) with GradScaler
  - Muon Newton-Schulz kept in fp32 (fp16 overflows)
  - flash_sdp → mem_efficient + math (flash requires SM80+)
  - GQA: manual KV-head expansion (T4 SDP lacks enable_gqa)
  - fused=True → fused=False on Adam
  - Single-GPU mode forced (free tier = 1 GPU)
  - Checkpoint save/load for surviving Colab disconnects

Do not edit this file directly — regenerate from train_gpt.py when the
base script changes.
"""
...
class Hyperparameters:
    ...
    activation_type = os.environ.get("ACTIVATION_TYPE", "relu2").lower().replace("-", "_")
...
def resolve_attention_backend(requested: str) -> str:
    if requested not in {"auto", "sdpa", "flash_attn_3"}:
        raise ValueError(f"Unsupported ATTENTION_BACKEND={requested}")
    if requested == "sdpa":
        return requested
    if requested == "flash_attn_3":
        if flash_attn_3_func is None:
            raise RuntimeError("ATTENTION_BACKEND=flash_attn_3 requires flash_attn_interface to be installed")
        return requested
    if flash_attn_3_func is not None and torch.cuda.is_available() and torch.cuda.get_device_capability()[0] >= 9:
        return "flash_attn_3"
    return "sdpa"
...
class MLP(nn.Module):
    # Keep the baseline relu^2 path by default, while allowing the lightweight
    # LeakyReLU^2 variant that is showing up in recent parameter-golf PRs.
    def __init__(self, dim: int, mlp_mult: int, activation_type: str, leaky_relu_slope: float):
        super().__init__()
        if activation_type not in {"relu2", "leaky_relu2"}:
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.fc(x)
        if self.activation_type == "leaky_relu2":
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, attention_backend)
        self.mlp = MLP(dim, mlp_mult, activation_type, leaky_relu_slope)
...
def main() -> None:
    ...
    log0(
        ...
        f"backend:{base_model.blocks[0].attn.attention_backend}"
    )
    log0(
        f"mlp_activation:{args.activation_type} "
        f"leaky_relu_slope:{args.leaky_relu_slope if args.activation_type == 'leaky_relu2' else 0.0}"
    )
    log0(
